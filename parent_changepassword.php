...
if(isset($_POST['changepwd']))
{
  ...
  $np=$_POST['newpassword'];
	...
	$sql="SELECT guardian_password FROM registration where id=?";
	$chngpwd = $mysqli->prepare($sql);
	$chngpwd->bind_param('s',$ai);
	$chngpwd->execute();
	$chngpwd->store_result(); 
    $row_cnt=$chngpwd->num_rows;;
	if($row_cnt>0)
	{
		$con="update registration set guardian_password=?, updationDate=? where id=?";
